...
def main() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input", required=True, help="Pad naar CSV orders (verplicht)"
    )
    parser.add_argument(
        "--output", default="rapport_<datum>.md", help="Pad voor het rapport"
    )
    parser.add_argument(
        "--format",
    ...
    if args.output != "rapport_<datum>.md":
        if args.format == "markdown":
            if not args.output.endswith(".md"):
                raise ValueError(
                    "De bestandsextentie komt niet overeen met het geselecteerde formaat"
                )

        elif args.format == "json" and not args.output.endswith(".json"):
            raise ValueError(
                "De bestandsextentie komt niet overeen met het geselecteerde formaat"
            )
